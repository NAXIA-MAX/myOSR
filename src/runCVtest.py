...
def knn_osr_detection(feat_id, feat_ood, label_id, label_ood, cv_round):
    """使用KNN进行OSR检测"""
    import faiss
    ...
    plt.tight_layout()
    plt.savefig(f"images/cv_round_{cv_round+1}_results.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def run_cross_validation(num_unknown=3, num_cv=5, data_path="/workspace/OSR/jammingData/3channels/", 
                        labels_file="labels_3channel.npy", data_file="data_3channel.npy"):
    """
    运行交叉验证实验
    ...
    for cv_round in range(num_cv):
        ...
        # ID数据：只包含已知类
        id_idx = np.isin(labels, known_classes)
        X_id = data[id_idx]
        y_id = labels[id_idx]
        ...
        # OOD数据：只包含未知类
        ood_idx = np.isin(labels, unknown_classes)
        X_ood = data[ood_idx]
        y_ood = labels[ood_idx]
        ...
        save_path = train_model(tailname, trainData, trainLabels, valData, valLabels,
                              saveStart_epoch=2, epochNum=5, numClasses=len(known_classes), 
        ...
        auroc, scores, labels = knn_osr_detection(
            feat_id, feat_ood, label_id, label_ood, cv_round
        )
    ...
    np.save("cv_results/cross_validation_results.npy", results_summary)
    ...
    plt.tight_layout()
    plt.savefig("images/cv_summary.png", dpi=300, bbox_inches='tight')
    plt.close()

    print(f"\n结果已保存到:")
    print(f"- 详细结果: cv_results/cross_validation_results.npy")
    print(f"- 汇总图表: images/cv_summary.png")
