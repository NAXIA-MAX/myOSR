...
def main(s):
    ...
    # 加载数据集
    fileFolder="/workspace/OSR/jammingData/3channels/"
    trainedfolder="trainedPth/trainedCVpth/"
    ...
    # 使用 dummy 输入获取特征维度
    if inputChannel==3:
        dummy_input = torch.zeros(1, inputChannel, datashpae[0].shape[1],datashpae[0].shape[2]).to(device)
    elif inputChannel==1:
        dummy_input = torch.zeros(1, inputChannel, datashpae[0].shape[0],datashpae[0].shape[1]).to(device)
    with torch.no_grad():
        _ = model(dummy_input)
